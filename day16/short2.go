...
func main() {
	...
	var next func(va, vb *valve, ta, tb int) int
	next = func(va, vb *valve, ta, tb int) int {
		tb--
		if tb <= 0 {
			return 0
		}
		...
		for i := range valves {
			...
			nextmax := next(vb, nv, tb, ta-shortest[va.name+nv.name]) // va → nv (next valve), vb → va
			if nextmax > max {
				max = nextmax
			}
			valves[i].visited = false
		}
		return max + vb.rate*tb
	}

	println(next(&valve{name: "AA", rate: 0}, &valve{name: "AA", rate: 0}, 26, 27))
}
